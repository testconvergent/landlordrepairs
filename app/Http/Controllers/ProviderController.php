...
use App\Mail\providerMarkCompletedMail;
...
class ProviderController extends BaseController
{
    ...
    public function getProviderDetails($userId){
        ...
        if (!@$provider_details->prof_image) {
            $provider_details->prof_image='blank-profile-picture.png';
        }
    ...
    public function request_feedback(Request $request){
        if ($request->isMethod('post')) {
            ...
            Mail::to($customer_email)->send(new requestFeedback($request_feedback));
    ...
    public function provider_mark_complete_job(Request $request){
        if ($request->isMethod('post')) {
            ...
            Mail::to($customer_email)->send(new providerMarkCompletedMail($request_complete));
    ...
    public function prof_pic_upload(Request $request)
    {
        ...
        if (@$request->all()) {
            ...
            $user_details = User::find($userId);
            if (@$user_details->prof_image) {
                unlink($destinationPath.'/'.$user_details->prof_image);
            }
    ...
    public function my_credits(){
        ...
        $current_package=OrderDetails::member($user_id)
                        ->whereHas('packageDetails', function($query) {
                            $query->where('package_type',2);
                        })
                        ->orderBy('order_id','desc')
                        ->first();      
        $is_renewed=false;
        if (!count($current_package)) {
            $previous_package=OrderDetails::member($user_id)
                ->previousPackage()
                ->with('packageDetails')
                ->first();
        ...
        //if the builder without any credit package
        if(count($package)){
            $data['package_name']=$package->packageDetails->credit_type_package_name;
            $data['price']=$package->amount;
            $data['credit_receive']=$package->packageDetails->credit_point;
            $data['is_renewed']=$is_renewed;
            return view('builder_credits', $data);
        } else{
            $data['creditPackages']= Package::where('package_status',1)->where('package_type',2)->get();           
            return view('builder_credits_without_package',$data);
        }
    }

    public function change_card_details(Request $request){
        $card_details=$request->all();
        if (@$request->all()) {
            $user_id=session()->get('user_id');
            //update the previous card status inactive mode
            PaymentVault::user($user_id)->update(['status' => 0]);
            //insert new card status inactive mode
            $vault_details=new PaymentVault;
            $vault_details->owner_name=$request->name_on_card;
            $vault_details->card_no=$request->card_no;
            $vault_details->card_cvv=$request->cvvNumber;
            $vault_details->card_expiry_month=$request->ccExpiryMonth;
            $vault_details->card_expiry_year=$request->ccExpiryYear;
            $vault_details->order_no='';
            $vault_details->user_id=session()->get('user_id');
            $vault_details->save();
    ...
    public function renew_package(){
        ...
        $previous_package=OrderDetails::member($user_id)
                        ...
                        ->with('packageDetails')
                        ->first();
        $packageDetails=Package::find($previous_package->packageDetails->package_id);
        if ($packageDetails->package_type==1) {
            $package_name='Bronze';
        }
        if ($packageDetails->package_type==2) {
            $package_name='Silver';
        }
        ...
        $data['price']=$packageDetails->cost;
        $data['credit_receive']=$packageDetails->credit_point;
        $package_period=$packageDetails->period;
        $package_obj=new PackageOrderDetails();
        $package_period=$package_obj->calculate_package_period($package_period);
        $data['valid_from']=$package_period['package_start'];
        $data['valid_to']=$package_period['package_start'];
        $data['saved_card_details']=PaymentVault::userActiveCard($user_id)->first();
        return view('builder_renew_package', $data);
    }
}
